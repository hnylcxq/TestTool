...
extern "C" void scl_stream_c(int src_w, int src_h, int dst_w, int dst_h, 
...
extern "C" void scl_panel_c(int src_w, int src_h, int dst_w, int dst_h, 
 ...
 */
extern "C" void csc_plane_c(int width, int height,
 ...
 * - Inside the function, R/B data is switched for CSC operation.
 */
extern "C" void csc_cursor_c(int width, int height,
                  unsigned outfmt,
                  const unsigned char* psrc, unsigned char* pdst,
                  std::string ccv_ckp_fn);

extern "C" void input_c(unsigned short* p_src, unsigned short* p_dst, 
    unsigned int sf_w, unsigned int sf_h, unsigned int sfmt, inp_para para, 
    std::string swv_ckp_fn);

extern "C" void cursor_c(unsigned short* p_cur, cur_para para, 
    unsigned short* p_cursor_out, int* w_out, int* h_out, 
    std::string cv_ckp_fn);

extern "C" void lut_c(unsigned short* p_src, unsigned short* p_dst, 
    unsigned int wid, unsigned int het, lut_para para, uint32_t* table,
    std::string li_ckp_fn, std::string lc_ckp_fn);

extern "C" void dither_c(unsigned short* p_src, unsigned short* p_dst, 
    unsigned int wid, unsigned int het, dither_para para, 
    std::string did_ckp_fn, std::string dc_ckp_fn);

extern "C" void signature_c(int width, int height, const unsigned short* psrc, unsigned short* pdst);

extern "C" unsigned long long signature_drv_c(int width, int height, const unsigned short* psrc);

extern "C" void output_c(unsigned short* p_src, unsigned short* p_dst,
