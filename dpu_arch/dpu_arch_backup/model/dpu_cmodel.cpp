...
void set_color_to_bus(unsigned short* pbus_, unsigned short* pcolor, bool* is_color_key = nullptr, bool* is_alpha_key = nullptr)
{
...
bool check_color_key(uint16_t* pick_col, unsigned int dst_w, unsigned int keyl, unsigned int keyh, unsigned int key_mode, bool &is_color_key, bool &is_alpha_key)
{
    ...
    if ((key_mode & 3) == 3)
    {
        ...
        {
            is_color_key = is_color_key && pick_col[0] >= ((keyl >> shift) & 0x3ff) && pick_col[0] <= ((keyh >> shift) & 0x3ff);
        }
    }
...
void ovl_cur(unsigned char* cur_pix, int ovl_type, unsigned short* a_pix, unsigned short* out, bool is_cur_ycbcr, bool is_a_cbcr, bool& is_res_cbcr, bool is_valid)
{
    ...
    for (int ch_id = 0;ch_id < 3;ch_id++)
    {
        ...
        default: //blending ovl_type = CUR_BLD_PRE or CUR_BLD
            ...
            int lsb = ((cur_pix[3] & 0x80) == 1) ? (a_pix[ch_id] & 3) : 0;
            ...
            res = (cur_s * alpha + alpha_s_inv * a_s + bld_rounding)>>8;
    ...
    for (uint32_t y = 0; y < h; y++)
    {
        ...
        while (x < w)
        {
            ...
            else if (sfmt == 3) // yuyv & uyvy
            {
                ...
                set_color_to_bus(p_dst + y * w * 4 + x * 4, ch);
                dump_tag.spl_unpack_dat = make_bus_data_for_dump(ch);
                ck_dump(&dump_tag);
                set_color_to_bus(p_dst + y * w * 4 + (x + 1) * 4, ch1);
...
{
    para.panel_width = 200;
    para.panel_height = 200;
    para.cur_size = 1;
    para.x_off = 63;
    para.y_off = 127;
    para.x_start = 200;
    ...
    if (x_off_on_panel + cur_w > para.panel_width)
    {
    ...
    if (y_off_on_panel + cur_h > para.panel_height)
    {
    ...
    for (uint32_t y = 0; y < cur_h; y++)
    {
        ...
        for (uint32_t x = 0; x < cur_w; x++)
        {
            ...
            if (para.cur_type == 0) // mono cursor
            {
                ...
                uint32_t d_a = ((d & 0xff) >> off) & 0x1;
                uint32_t d_x = (((d >> 16) & 0xff) >> off) & 0x1;
